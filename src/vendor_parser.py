...
class VendorQueryBuilder:
    ...
    def build_basic_vendor_query(self, indices: List[str], time_range: str = '-24h@h') -> str:
        """
        ...
        for vendor, patterns in self.vendor_patterns.items():
            pattern_conditions = ' OR '.join([f'match(_raw, "(?i){pattern}")' for pattern in patterns])
            vendor_cases.append(f'({pattern_conditions}), "{vendor.replace("_", " ").title()}"')
...
| sort -count'''
    ...
    def build_custom_query(self, indices: List[str], custom_patterns: Dict[str, List[str]], 
                          time_range: str = '-24h@h', additional_filters: Optional[str] = None) -> str:
        """
...
| sort vendor_product, -count'''
    ...
    def build_gap_analysis_query(self, indices: List[str], expected_vendors: List[str], 
                                time_range: str = '-7d@d') -> str:
        """
        ...
        filtered_patterns = {k: v for k, v in self.vendor_patterns.items() 
...
| sort vendor_product, index'''
    ...
    def extract_vendor_products(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['percentage'] = (df['count'] / total_events * 100).round(2) if total_events > 0 else 0
    ...
    def identify_coverage_gaps(self, vendor_data: pd.DataFrame, expected_vendors: List[str]) -> pd.DataFrame:
        """
        ...
        expected_vendors_lower = [v.lower() for v in expected_vendors]
        ...
        for expected_vendor in expected_vendors:
            ...
            if vendor_lower in present_vendors:
                # Vendor is present
                vendor_row = vendor_data[vendor_data['vendor_product'].str.lower() == vendor_lower].iloc[0]
                gap_analysis.append({
                    'vendor_product': expected_vendor,
                    'status': 'Present',
                    'event_count': int(vendor_row['count']),
                    'percentage': float(vendor_row.get('percentage', 0)),
                    'gap_severity': self._assess_gap_severity(vendor_row['count'], vendor_data['count'].sum())
                })
            else:
                # Vendor is missing
                gap_analysis.append({
    ...
    def _classify_vendor_category(self, vendor_name: str) -> str:
        ...
        if any(term in vendor_lower for term in ['cisco', 'juniper', 'palo alto', 'fortinet', 'checkpoint']):
    ...
    def generate_summary_stats(self, vendor_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'top_vendor': vendor_data.iloc[0]['vendor_product'] if not vendor_data.empty else 'None',
            'top_vendor_count': int(vendor_data.iloc[0]['count']) if not vendor_data.empty else 0,
            'categories': category_stats,
            'unknown_percentage': vendor_data[vendor_data['vendor_product'] == 'Unknown']['percentage'].sum()
        }