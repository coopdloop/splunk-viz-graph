...
class SplunkHECClient:
    ...
    def validate_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            url = self.base_url.replace(
                "/services/search/jobs", "/services/server/info"
            )
    ...
    def execute_search(self, search_query: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(
                url, data=search_params, verify=self.verify_ssl, timeout=60
            )
    ...
    def get_search_results(
        self, job_id: str, offset: int = 0, count: int = 1000
    ) -> Dict[str, Any]:
        """
        ...
        try:
            response = self.session.get(
                url, params=params, verify=self.verify_ssl, timeout=60
            )
            response.raise_for_status()
    ...
    def wait_for_job_completion(
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                if dispatch_state == "DONE":
                    return {"success": True, "completed": True, "job_data": job_data}
        ...
        return {"success": False, "error": "Job did not complete within timeout period"}
    ...
    def paginate_results(
        self, query: str, page_size: int = 1000, max_results: Optional[int] = None
    ) -> pd.DataFrame:
        """
